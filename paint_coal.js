class Paint {
    ...
    _start(shaderSources) {
      ...
      // --- simple outline fragment for paintingRectangle ---
      ...
      // High-fidelity raw updates where supported
      if ('onpointerrawupdate' in window) {
        canvas.addEventListener('pointerrawupdate', this.onPointerMove.bind(this), { passive: false });
      }
      // Initialize brush on entry to restore "appears on first move"
      canvas.addEventListener('pointerenter', this.onPointerEnter.bind(this), { passive: true });
      ...
      if (this.saveButton) {
        this.saveButton.addEventListener('click', () => this.save && this.save());
      ...
      // --- painting rectangle outline defaults ---
      this.showPaintingRect = true;
      this.paintingRectThickness = 2.0;
      // #0ea5e9 -> (14, 165, 233)
      this.paintingRectColor = [14/255, 165/255, 233/255, 1];
    ...
    update() {
      ...
      // --- Brush update with high-res motion history ---
      if (this.brushInitialized) {
        if (this.pendingSamples.length) {
          for (let i = 0; i < this.pendingSamples.length; i++) {
            const s = this.pendingSamples[i];
            this.brushX = s.x;
            this.brushY = s.y;
          ...
          // consumed all buffered samples
          this.pendingSamples.length = 0;
        } else {
          // keep brush alive for hover / idle frames
          this.brush.update(
            this.brushX,
            this.brushY,
            BRUSH_HEIGHT * this.brushScale,
      ...
      // --- draw the paintingRectangle outline ---
    ...
    onPointerDown(event) {
      ...
      // fresh gesture: reset buffered samples
      this.pendingSamples.length = 0;
      ...
      // Seed the brush so first velocity is zero (prevents bottom-left snap)
      if (!this.brushInitialized) {
    ...
    onPointerMove(event) {
      // Prevent default for touch/pen or while dragging; allow mouse hover
      if (event.pointerType !== 'mouse' || event.buttons) {
        if (event.preventDefault) event.preventDefault();
      }
  
      // Gather all coalesced samples (includes base event)
      const list = (typeof event.getCoalescedEvents === 'function')
        ? event.getCoalescedEvents()
        : [event];
  
      // Enqueue all samples for high-res playback in update()
      for (const e of list) {
        const p = Utilities.getMousePosition(e, this.canvas);
        const x = p.x;
        const y = this.canvas.height - p.y;
  
        // Track pointer
        this.activePointers.set(event.pointerId, { x, y, type: event.pointerType });
  
        // For hover/primary, keep live brush coords refreshed
        const drivingId = this.primaryPointerId ?? event.pointerId;
        if (event.pointerType === 'mouse' || event.pointerId === drivingId) {
          this.brushX = x;
          this.brushY = y;
        }
  
        // Queue sample
        this.pendingSamples.push({ x, y, id: event.pointerId, t: e.timeStamp });
      }
  
      // Color picker hover uses last reported point
      const last = list[list.length - 1];
      const lp = Utilities.getMousePosition(last, this.canvas);
      const mx = lp.x;
      const my = this.canvas.height - lp.y;
  
      // Compute deltas for panning/resizing using the last point this frame
      ...
      // Ensure brush becomes visible on first move even without click
      if (!this.brushInitialized) {
        this.brush.initialize(mx, my, BRUSH_HEIGHT * this.brushScale, this.brushScale);
        this.brushInitialized = true;
      }
    }
    ...
    onPointerUp(event) {
      ...
      // Treat color picker up using last known coords
    ...
    onPointerEnter(event) {
      const pos = Utilities.getMousePosition(event, this.canvas);
      const x = pos.x;
      const y = this.canvas.height - pos.y;
      ...
      if (!this.brushInitialized) {
        this.brush.initialize(x, y, BRUSH_HEIGHT * this.brushScale, this.brushScale);
        this.brushInitialized = true;
      }
    }
