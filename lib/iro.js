...
(function (global, factory) {
        ...
        function () {
            ...
            */
            ...
            IroColor.hsvToRgb = function hsvToRgb(hsv) {
                /* @creg PIGMENT: this picker drives a SUBTRACTIVE paint model.
                 *
                 * Upstream this was the plain sexagesimal HSV->RGB below, which
                 * is right for a light-mixing picker and wrong for this one. The
                 * simulation takes the picked hue through hsvToRyb() and then
                 * the RYB pigment cube, and that pair is NOT a round trip -- so
                 * the colour this function returned was not the colour the brush
                 * deposited. Measured across the wheel:
                 *
                 ...
                 *
                 * Red was the only fixed point; everything else sat about
                 * 120deg from the pigment. So the wheel NAMED a different colour
                 * from the one about to come out of the brush.
                 *
                 * Routing through hsvToPigmentRgb() (app/ui/ryb.js) fixes every
                 * surface at once, because the wheel gradient, the slider
                 * gradients, the handle fills and `color.rgb` all derive from
                 * here. That is why the change belongs at this function rather
                 * than being painted over each surface from outside.
                 *
                 * The inverse, rgbToHsv, is deliberately NOT changed: its only
                 * caller is the `rgb` setter, which this app never uses (it sets
                 * {h,s,v,a}). If an RGB setter is ever wanted here, it needs a
                 * real inverse of the cube, not this function's old body.
                 */
                if (typeof hsvToPigmentRgb === 'function') {
                    var pigment = hsvToPigmentRgb(
                        hsv.h / 360, hsv.s / 100, hsv.v / 100,
                        typeof iroAdditiveModel === 'function' ? iroAdditiveModel() : false
                    );
                    return {
                        r: clamp(pigment[0] * 255, 0, 255),
                        g: clamp(pigment[1] * 255, 0, 255),
                        b: clamp(pigment[2] * 255, 0, 255)
                    };
                }
            ...
            _createClass(IroColor, [{
                ...
                key: "hslString",
                get: function get() {
                    /* @creg PIGMENT: returns the pigment colour, not hsl().
                     *
                     * This getter is what fills the wheel and box HANDLES
                     * (IroHandle's `fill` prop). It reached the colour through
                     * `this.hsl` -> IroColor.hsvToHsl, a SEPARATE additive path
                     * that never touches hsvToRgb -- so converting hsvToRgb
                     * left the handles alone and a handle sitting on the ring
                     * showed a different colour from the ring underneath it.
                     *
                     * `this.rgb` is already the pigment (see hsvToRgb above),
                     * so returning it here makes the handle agree with every
                     * other surface. The name stays `hslString` because that is
                     * what the widgets ask for; only the space changes. CSS does
                     * not care which function produced the colour.
                     */
                    var rgb = this.rgb;
        ...
        switch (props.sliderType) {
            ...
            case 'hue':
                ...
                for (var hi = 0; hi <= 24; hi++) {
                    var ht = hi / 24;
                    var hc = IroColor.hsvToRgb({ h: ht * 360, s: 100, v: 100 });
            ...
            case 'saturation':
                ...
                for (var si = 0; si <= 8; si++) {
                    var st = si / 8;
                    var sc = IroColor.hsvToRgb({ h: hsv.h, s: st * 100, v: hsv.v });
            ...
            default:
                ...
                for (var vi = 0; vi <= 8; vi++) {
                    var vt = vi / 8;
                    var vc = IroColor.hsvToRgb({ h: hsv.h, s: hsv.s, v: vt * 100 });
    ...
    function iroHueGradient(anticlockwise) {
        ...
        for (var i = 0; i <= STOPS; i++) {
            ...
            var c = IroColor.hsvToRgb({ h: hue, s: 100, v: 100 });
