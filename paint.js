...
class Paint {
  ...
  _start(shaderSources) {
    ...
    if (this.saveButton) {
      ...
      this.saveButton.addEventListener('touchstart', (event) => {
    ...
    if (this.clearButton) {
      ...
      this.clearButton.addEventListener('touchstart', (event) => {
        event.preventDefault();
        this.clear();
      });
    }
    ...
    if (this.undoButton) {
      ...
      this.undoButton.addEventListener('touchstart', (event) => {
        event.preventDefault();
        this.undo();
      });
    }
    ...
    if (this.redoButton) {
      ...
      this.redoButton.addEventListener('touchstart', (event) => {
        event.preventDefault();
        this.redo();
      });
    }
    ...
    if (this.interactionState === InteractionMode.PANNING) {
      ...
      if (
        ...
        this.newPaintingRectangle.width = Utilities.clamp(
          mouseX - this.paintingRectangle.left,
          MIN_PAINTING_WIDTH,
          this.maxPaintingWidth
        );
      }
      ...
      if (
        ...
        this.newPaintingRectangle.height = Utilities.clamp(
          mouseY - this.paintingRectangle.bottom,
          MIN_PAINTING_WIDTH,
          this.maxPaintingWidth
        );
      }
  ...
  onTouchEnd(event) {
    event.preventDefault();
    if (event.touches.length > 0) return; // don't fire if touches remain
    this.onMouseUp({});
  }
